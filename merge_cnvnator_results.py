...
class cluster_details:
  ...
  #set functions
	...
	def print_calls(self):
		str_temp = self.chrm + "*" +   `self.start` + "*" + `self.end` + "*" + self.cnv + "*" +`self.num_var` + "*" + `self.len_var` + "*" + `self.len_gap` + "*" + "|".join(self.nrd_tag.keys()) + "##"
...
#t_file_name = TCGA-05-5429-01A-01D-1625-08.calls.txt.temp  
  ...
  for c in merged.keys():                
    for o in merged[c]:
      ...
      f_line = sample_name + "\t" + o.get_chrm() + "\t" + `o.get_start()` + "\t" +`o.get_end()` + "\t" + o.get_cnv() + "\t" + `o.get_size()` + "\t" + o.get_nrd_tag() +"\t-\t-\t" + `o.get_num_var()` + "\t" + `o.get_len_var()` + "\t" + `o.get_len_gap()` +"\t"+`(o.get_len_gap()/(o.get_size()*1.0))*100`
      print >> o_file, f_line
